...
import { devDrawCard, disconnectFromMatch, fetchMatch, joinMatch, playCard, requestAddBot, requestStartMatch, requestUpdateExpansion, respondToPendingAction } from "../api/gameApi";
...
import { loadStoredLanguage, localizeMatchState, persistLanguage, t, type AppLanguage } from "../i18n";
...
import type { CardView, ExpansionKey, GameEvent, MatchState, SeatState } from "../../../shared/types";
...
function renderTableScene(
  ...
  const currentTurnSeatNumber = match.game?.currentTurnSeatNumber;
  ...
  const responseCards = pendingAction?.responders
    .flatMap((responder) => responder.cards ?? (responder.card == null ? [] : [responder.card]))
    .filter((card) => getResponseChoiceForCard(card) != null)
    ?? [];
  const displayedCenterCard = pendingAction?.card ?? lastPlayedCard ?? null;
  const centerStackCards = [
    ...(pendingAction != null && lastPlayedCard != null && lastPlayedCard.instanceId !== pendingAction.card.instanceId
      ? [lastPlayedCard]
      : []),
  ...
  anchors.forEach((anchor, index) => {
    ...
    const seatX = anchor.x * STAGE_WIDTH / 100;
  ...
  for (const [seatNumber, rect] of seatRects.entries()) {
    if (impactTargetSeatNumbers.includes(seatNumber)) {
      scene.addChild(createRect(rect.x - 8, rect.y - 8, rect.width + 16, rect.height + 16, "#c94b4b", 0.18, 28));
    }

    const damageAmount = activeDamageBursts[seatNumber];
    if (damageAmount != null) {
      scene.addChild(createLabel(`-${damageAmount} HP`, rect.x + rect.width / 2, rect.y - 18, {
    ...
    const healAmount = activeHealBursts[seatNumber];
    if (healAmount != null) {
      scene.addChild(createLabel(`+${healAmount} HP`, rect.x + rect.width / 2, rect.y - 18, {
        fontSize: seatNumber === localSeatNumber ? 30 : 22,
        fontWeight: "700",
        fill: "#b9f3c5"
      }, 0.5, 0.5));
  ...
  if (pendingAction != null) {
    ...
    const actorCenter = seatCenters.get(pendingAction.actorSeatNumber);
    const actorRect = seatRects.get(pendingAction.actorSeatNumber);
    ...
    if (pendingAction.targetObjectInstanceId != null) {
      const objectInspectTarget = inspectTargets.find((target) =>
        target.group === "object" && target.card.instanceId === pendingAction.targetObjectInstanceId
      );
...
function buildOverlayMarkup(
  ...
  return `
    ...
    ${activeCombatFx == null
      ? ""
      : `<div class="pixi-combat-banner pixi-combat-banner--${activeCombatFx.tone}">${escapeHtml(activeCombatFx.message)}</div>`}
    ${showPassButton
      ? `
...
export async function createPixiApp(rootElement: HTMLDivElement): Promise<void> {
  ...
  let activeDamageBursts: Record<number, number> = {};
  let activeHealBursts: Record<number, number> = {};
  let impactTargetSeatNumbers: number[] = [];
  ...
  const hasActiveLocalInteraction = (): boolean =>
    interactionState.draggingCardInstanceId !== "" || interactionState.arrowDrag != null || overlayInteractionLocked;
  ...
  const performCardPlay = async (request: Parameters<typeof playCard>[2]): Promise<void> => {
    try {
      match = await playCard(session.instanceId, playerSessionToken, request);
      errorMessage = "";
    } catch (error) {
  ...
  const performPendingResponse = async (
    ...
    try {
      match = await respondToPendingAction(session.instanceId, playerSessionToken, request);
      errorMessage = "";
      pendingAnnulationChoice = null;
    } catch (error) {
      errorMessage = error instanceof Error ? error.message : t(language, "error.playCard");
  ...
  const redraw = (): void => {
    ...
    frameElement.innerHTML =
      leftMessage !== ""
        ...
        `
        : buildOverlayMarkup(localizedMatch, localSeatNumber, language, errorMessage, pendingAnnulationChoice, activeCombatFx, passButtonLeftPx, passButtonTopPx);
  ...
  const requestSync = async (): Promise<void> => {
    ...
    syncInFlight = true;
    try {
      match = await fetchMatch(session.instanceId, playerSessionToken);
      errorMessage = "";
  ...
  const bindOverlayEvents = (): void => {
    ...
    frameElement.querySelector<HTMLButtonElement>("[data-action='leave-match']")?.addEventListener("click", async () => {
      try {
  ...
  const handleCanvasPointerMove = (event: PointerEvent): void => {
    if (!currentMetrics.isLandscape || leftMessage !== "") {
      return;
    }
  ...
  const handleCanvasPointerDown = (event: PointerEvent): void => {
    if (event.button !== 0 || !currentMetrics.isLandscape || leftMessage !== "") {
      return;
    }
